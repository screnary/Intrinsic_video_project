...
def visualize_inspector(inspector, save_dir, step_num=None, mode='Basic'):
    fig_n = 7+3  # subplot blocks number
    ...
    loss_flow = inspector['loss_flow'][::sample_step]
    loss_flow_r = inspector['loss_flow_r'][::sample_step]
    loss_flow_s = inspector['loss_flow_s'][::sample_step]
    ...
    ax8.plot(x, loss_flow, '.-')
    ax8.set_ylabel('loss-flow')

    ax9 = f.add_subplot(fig_n, 1, 9)
    ax9.plot(x, loss_flow_r, '.-')
    ax9.set_ylabel('loss-flow-r')

    ax10 = f.add_subplot(fig_n, 1, 10)
    ax10.plot(x, loss_flow_s, '.-')
    ax10.set_ylabel('loss-flow-s')

    last_n = 11
    if len(inspector['loss_preserve_info']) > 0:
        ax11 = f.add_subplot(fig_n, 1, 11)
        ax11.plot(x, loss_preserve_info, '.-')
        ax11.set_ylabel('loss_preserve_info')
        last_n = 12
