...
from networks_mpi_video import get_generator, get_discriminator, GANLoss, DivergenceLoss, PerspectiveLoss
from networks_mpi_video import Grad_Img_v1 as Grad_Img
import pytorch_ssim
from my_data import Warper2d
import copy
import pdb
...
class Trainer_Basic(nn.Module):
    @staticmethod
    def name():
        return 'IID_Trainer'  # intrinsic image decomposition
    ...
    def __init__(self, t_opt):
        ...
        self.criterion_cos_sim = torch.nn.CosineSimilarity(dim=1,
                                                           eps=1e-11).cuda()
        self.warp_op = Warper2d((t_opt.data.image_height, t_opt.data.image_width)).cuda()
    ...
    def forward(self):
        ...
        if self.is_train:  # for calculating of flow based identity error
            fake_s2, fake_r2, _ = self.gen_split(self.padimg(self.real_i2))[:3]
            self.fake_s2 = fake_s2[:,:,self.padedge:-self.padedge,
                                self.padedge:-self.padedge].repeat(1, 3, 1, 1)
            self.fake_r2 = fake_r2[:,:,self.padedge:-self.padedge,
                                self.padedge:-self.padedge]

    def set_input(self, input_data):
        input_i = input_data['input_1'].float()
        input_s = input_data['shading_1'].float()
        input_r = input_data['albedo_1'].float()

        self.img_name = input_data['filename']
        ...
        # if in train phase, calculate flow_based identity
        if self.is_train:
            input_i2 = input_data['input_2'].float()
            input_s2 = input_data['shading_2'].float()
            input_r2 = input_data['albedo_2'].float()
            flow_data = input_data['flow'].float()
            occ_mask = input_data['occ'].float()

            self.input_i2 = input_i2.cuda()
            self.input_s2 = input_s2.cuda()
            self.input_r2 = input_r2.cuda()
            self.flow_data = flow_data.cuda()
            self.occ_mask = occ_mask.cuda()

            self.real_i2 = Variable(self.input_i2)
            self.real_s2 = Variable(self.input_s2)
            self.real_r2 = Variable(self.input_r2)
    ...
    def loss_basic_computation(self):
        ...
        # [2021.03, video flow loss]
        if self.is_train and weight.flow_idt_w > 0:
            self._compute_flow_loss()
            self.loss_flow = self.loss_flow_r * weight.lambda_flow_r +\
                             self.loss_flow_s * weight.lambda_flow_s
        else:
            self.loss_flow_r = self.Tensor([0.0])
            self.loss_flow_s = self.Tensor([0.0])
            self.loss_flow = self.Tensor([0.0])
        ...
        self.loss_gen_basic = weight.identity_w * self.loss_gen_idt + \
                              ...
                              weight.flow_idt_w * self.loss_flow
    
    def _compute_flow_loss(self):
        warped_s2 = self.warp_op(self.flow_data, self.fake_s2)
        warped_r2 = self.warp_op(self.flow_data, self.fake_r2)
        valid_mask = 1.0-self.occ_mask
        # pdb.set_trace()
        self.loss_flow_s = self.criterion_idt(self.fake_s * valid_mask, warped_s2 * valid_mask)
        self.loss_flow_r = self.criterion_idt(self.fake_r * valid_mask, warped_r2 * valid_mask)
    ...
    def get_current_errors(self):
        ...
        ret_errors['flow_r'] = self.loss_flow_r
        ret_errors['flow_s'] = self.loss_flow_s
        ...
        log_detail = '\
                ...
                \t{}:{}, {}:{}, {}:{}\n \
                      ...
                      \t{}'.format(
                                   ...
                                   'loss_flow_r', losses['flow_r'],
                                   'loss_flow_s', losses['flow_s'],
                                   losses['img_name'])
        return log_detail
