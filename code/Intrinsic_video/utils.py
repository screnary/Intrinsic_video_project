...
from data import IntrinsicImageFolder
import torch
import os
import math
...
def get_intrinsic_data_loader(conf, mode='train'):
    data_root = conf['data_root'].replace('train', mode)
    is_shuffle = True if mode == 'train' else False
    dataset = IntrinsicImageFolder(root=data_root, is_shuffle=is_shuffle,
                                   new_size=conf['new_size'], mode=mode)
